...
a = imgs_anns["annotations"]
for anno_key in imgs_anns["annotations"]:
    # print(anno_key)
    key = anno_key["keypoints"]

    # print(len(key)) # 96
    # a[start : end : step]
    points = []
    try:
        # annotation에서 나온 결과를 4개씩 슬라이싱으로 만들기 위해서 for문 사용
        idx = 0
        for keys in range(16): # 3개의 포인트를 사용하여 2개의 x, y를 찾기 위해서 16개의 포인트를 가져온다

            point = key[0 + idx: 2 +idx]
            idx += 6
            for i in point:
                # print(i)
                points.append(i)
