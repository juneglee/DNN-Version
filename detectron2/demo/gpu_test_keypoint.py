...
# iloc를 행렬 형태로 만들어주고
# 인덱스를 numpy 형태의 리스트로 변경
motions = df.iloc[:, 1:] # 각각의 값들을 인덱스화(행렬) # [4195 rows x 48 columns]
columns = motions.columns.to_list()[::2] # 각각의 columns 값
class_labels = [label.replace('_x', '').replace('_y', '') for label in columns] # columns값을 x와 y를 제거

keypoints = []
for motion in motions.to_numpy(): # 각 인덱스 값들을 리스형태로 변경
    a_keypoints = []
    for i in range(0, motion.shape[0], 2): # x,y와로 담기 위해
        a_keypoints.append((float(motion[i]), float(motion[i+1])))
        # print(a_keypoints)
    keypoints.append(a_keypoints)
keypoints = np.array(keypoints)
print(keypoints) # 리스트 형태를 array로 변경


def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, is_inception=False):
    # criterion : Setup the 'loss' fxn
    # is_inception : is_inception flag
    since = time.time()
...
# print(test_loader) # <torch.utils.data.dataloader.DataLoader object at 0x0000025603DCA358>
...
# print(all_predictions)
# print(all_predictions.shape[0]) # 1600
