...
def get_parser():
    ...
    parser.add_argument("--video-input",
        # add file name
        # default="../assert/test/shopping_mall.small.mp4",
...
if __name__ == "__main__":
    ...
    #     # demo = VisualizationDemo(cfg)
    #     if len(args.input) == 2:
    #         args.input = glob.glob(os.path.expanduser(args.input[i]))
    #         # args.input2 = glob.glob(os.path.expanduser(args.input[1]))
    #         assert args.input, "The input path(s) was not found"
    #
    #     for path in tqdm.tqdm(args.input, disable=not args.output):
    #         # use PIL, to be consistent with evaluation
    ...
    #                 break  # esc to quit
            ...
            # for path in tqdm.tqdm(args.input2, disable=not args.output):
            #     # use PIL, to be consistent with evaluation
            #     img = read_image(path, format="BGR")
            #     start_time = time.time()
            #     predictions, visualized_output = demo.run_on_image(img)
        ...
        # webcam input
        # webcam & image를 동시에 사용 불가능
        # output support not yet
        # elif args.webcam:
